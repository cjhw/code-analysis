#!/usr/bin/env node
import { Command } from "commander/esm.mjs"; // 命令行交互
import path from "path"; // 路径操作
import fs from "fs"; // 文件操作
import chalk from "chalk"; // 美化输出
import { pathToFileURL } from "url";

import { writeReport, writeDiagnosisReport } from "../lib/report.js";
...
program
  ...
  .action(async () => {
    try {
      ...
      if (isConfig) {
        let configModule = await import(pathToFileURL(configPath));
